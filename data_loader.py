...
class DataLoader(object):
    ...
    def _random_crop(self, image, label):
        ...
        if min_size > self.image_size[0] and min_size > self.image_size[1]:
            image = tf.random_crop(image, [self.image_size[0], self.image_size[1], 3])
        else:
            image = tf.image.resize_image_with_crop_or_pad(image, self.image_size[0], self.image_size[1])
        return image, label
    ...
    def data_batch(self, augment=True, shuffle=True, batch_size=4, repeat_times=1, num_threads=1, buffer=30):
        """
        ...
        if self.image_size == [112, 112]:
            data = data.map(self._pre_resize_image, num_parallel_calls=num_threads).prefetch(buffer)
        ...
        data = data.map(self._resize_image, num_parallel_calls=num_threads).prefetch(buffer)
        data = data.map(self._image_standardization, num_parallel_calls=num_threads).prefetch(buffer)
